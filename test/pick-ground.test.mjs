...
const hookSrc = VIEWER_HOOK_JS;
{
  ...
  if (devtools) {
    // three.js observes Scenes through the same channel - the hook must not mistake one for
    // the renderer, and it must COLLECT them: the panel's viewer preloader reads the biggest
    // scene's child count off window.__scenes as its mesh-progress signal.
    const scene = { type: 'Scene', isScene: true };
    devtools.dispatchEvent({ type: 'observe', detail: scene });
    devtools.dispatchEvent({ type: 'observe', detail: scene });   // re-announced: no duplicate
    pass(Array.isArray(win.__scenes) && win.__scenes.length === 1 && win.__scenes[0] === scene,
      'the hook collects observed Scenes on __scenes (once each) for the loading overlay');
    const renderer = {
