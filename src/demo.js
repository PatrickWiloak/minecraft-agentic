...
import { fillRegion, blockBounds } from './fill.js';
...
async function demo() {
  ...
  // The real bounding box, not origin-to-max: a plan with a foundation course at y = -1 sits
  // BELOW the origin, and clearing from origin.y up left it standing. fillRegion also splits
  // the region across as many /fill commands as it takes - one caps at 32,768 blocks and is
  // refused silently above that (see src/fill.js).
  const b = blockBounds(plan.blocks, 1);
  await fillRegion(bot, b.x0, b.y0, b.z0, b.x1, b.y1, b.z1, 'air');
  await sleep(2000);
