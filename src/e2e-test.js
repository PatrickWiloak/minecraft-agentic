...
async function main() {
  ...
  // Clear then build. Through fillRegion like every other clear in the project, so there is
  // exactly one place that knows about the 32,768-block /fill cap (this box is nowhere near it,
  // but a raw /fill anywhere is how the rule gets forgotten again).
  await fillRegion(bot, origin.x, origin.y, origin.z, origin.x + 6, origin.y + 4, origin.z + 6, 'air');
  await sleep(500);
