...
// Extract and parse a JSON plan from a model response (handles ```json fences, and models
// that wrap the object in prose). The tolerant parser itself lives in src/json.js - it used
// to be written out here AND in repair.js, and two copies of the same parser is one too many.
function parsePlan(content) {
  try {
    return parseJsonish(content);
  } catch (err) {
    // The message names the actual failure (truncated / not JSON at all / unparseable), because
    // "did not return valid JSON" on its own has sent people to rewrite a prompt that was fine.
    console.error(`[Coordinator] Failed to parse plan: ${err.message}`);
    throw new Error(`The model's plan could not be read - ${err.message}`);
  }
}
