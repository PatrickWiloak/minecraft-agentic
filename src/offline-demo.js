...
async function runOfflineDemo() {
  ...
  const maxX = Math.max(...allBlocks.map(b => b.x)) + 1;
  const maxY = Math.max(...allBlocks.map(b => b.y)) + 1;
  const maxZ = Math.max(...allBlocks.map(b => b.z)) + 1;
  const minX = Math.min(...allBlocks.map(b => b.x)) - 1;
  const minZ = Math.min(...allBlocks.map(b => b.z)) - 1;

  firstWorker.bot.chat(`/fill ${minX} ${origin.y} ${minZ} ${maxX} ${maxY} ${maxZ} minecraft:air`);
  await sleep(2000);
