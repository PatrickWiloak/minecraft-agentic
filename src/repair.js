...
/**
 * Take a model's raw patch and make it safe to execute: absolute coordinates, a role we
 * actually have, a block type that exists as a string. A patch entry with no `type` goes out
 * as `/setblock x y z minecraft:undefined`, which the server rejects silently - the exact
 * class of quiet failure this whole module exists to catch.
 */
export function normalizePatch(patch, plan, limit = 80) {
  ...
  return {
    ...
    add: (patch.add || [])
      .filter((b) => ok(b) && typeof b.type === 'string' && b.type.trim())
      .map((b) => ({
        ...abs(b, origin),
        type: b.type.replace(/^minecraft:/, '').trim(),
        role: known.has(b.role) ? b.role : roles[0],
      }))
      .slice(0, limit),
    remove: (patch.remove || [])
      .filter(ok)
      .map((b) => ({ ...abs(b, origin), role: known.has(b.role) ? b.role : roles[0] }))
      .slice(0, limit),
  };
}
...
/** Same fence-tolerant JSON extraction the coordinator uses. */
